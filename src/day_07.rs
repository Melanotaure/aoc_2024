...
fn part2(input: &str) -> u64 {
    ...
    let mut accu = 0;
    for (i, res) in results.iter().enumerate() {
        if is_new_equ_ok(*res, &equations[i]) {
            accu += res;
        }
    }
    accu
...
fn is_new_equ_ok(result: u64, values: &Vec<u64>) -> bool {
    let mut res: Vec<(u64, u64, u64)> = Vec::new();
    res.push(add_mul_concat(values[0], values[1]));
    if values.len() > 2 {
        for i in 2..values.len() {
            let mut rtmp: Vec<(u64, u64, u64)> = Vec::new();
            for r in &res {
                let r1 = add_mul_concat(r.0, values[i]);
                let r2 = add_mul_concat(r.1, values[i]);
                let r3 = add_mul_concat(r.2, values[i]);
                rtmp.push(r1);
                rtmp.push(r2);
                rtmp.push(r3);
            }
    ...
    for r in res {
        if result == r.0 || result == r.1 || result == r.2 {
            return true;
        }
    }
    false
